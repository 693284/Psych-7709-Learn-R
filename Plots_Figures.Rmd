...
```{r}
library(ggplot2)
  study_group <- rep(as.factor(c("Doctor Study", "Butcher Study", "firefighter_study", "Construction Worker Study")),4)
  likely_occurance <- rep(c("women more likely", "equilly likely","man more likely"),3)
...
plot_df3 <- data.frame(study_group,
  ...
  ggplot(plot_df, aes(x=study_group,y=dv_means, # aestetics definition has gotten 
                    group=likely_occurance, 
                   # color=factor_two))+  # color here means the border collor
                    fill=likely_occurance))+
