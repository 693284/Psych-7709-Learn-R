...
Rtime_cong <- subject_meanRTs %>%
    ...
    SEMRtime_allsubj = sd(Rtime_incong$corr_RT - Rtime_cong$corr_RT)/sqrt(length(Rtime_cong$corr_RT))
    
    avgRtime = mean(abs(Rtime_incong$corr_RT - Rtime_cong$corr_RT))
    
    flankerdata <- data.table(avgRtime, SEMRtime_allsubj)
    
                                
    ggplot(flankerdata, aes(x="all_subj", y=avgRtime))+
